...
export const api = {
  async getResources(query = "") {
    ...
    return new Promise((res) => setTimeout(() => res(all.filter(r => r.title.toLowerCase().includes(query.toLowerCase()))), 350));
  },
  ...
  return (
    <div className="navbar">
      <div className="container nav-inner">
        <div className="logo"><div className="mark"/> DigiPsych</div>
        <div className="nav-links" aria-hidden>
          ...
          <div className="dropdown"><button>Resources</button></div>
          <div className="dropdown"><button>Peer Support</button></div>
          <div className="dropdown"><button>About</button></div>
          <div className="dropdown"><button>Contact</button></div>
        </div>
  ...
  return (
    <section className="container hero" id="home">
      <div className="hero-left">
        ...
        <p>AI-guided first aid, confidential counselling, peer support, and wellness resources — all in one place.</p>
        <div className="buttons">
          <button className="cta" onClick={onStart}>Start Now</button>
          <button className="btn-outline">Learn More</button>
        </div>
        <div style={{marginTop:18}}>
          <small style={{color:'var(--muted)'}}>Tailored for college students with regional language support and offline mapping to on-campus counsellors.</small>
        </div>
      </div>
      <div className="hero-right">
        <img src="/assets/hero_placeholder.png" alt="students using laptop" onError={(e)=>{e.target.src='data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"600\" height=\"360\"><rect width=\"100%\" height=\"100%\" fill=\"%230f2748\"/><text x=\"50%\" y=\"50%\" dominant-baseline=\"middle\" text-anchor=\"middle\" fill=\"%23bcd0ff\" font-size=\"18\">Hero Image Placeholder</text></svg>'}}/>
      </div>
    </section>
  );
}
  ...
  return (
    <div className="feature-card">
      <div style={{display:'flex',alignItems:'center',gap:12}}>
        <div style={{width:44,height:44,background:'rgba(255,255,255,0.03)',borderRadius:8,display:'grid',placeItems:'center'}}>{icon}</div>
        <div>
          <h4>{title}</h4>
          <p>{children}</p>
        </div>
  ...
  return (
    <section className="container">
      ...
      <div className="features" style={{marginTop:12}}>
        <FeatureCard icon={<Emoji label="robot">🤖</Emoji>} title="AI-guided Chatbot">Interactive first-aid chat that suggests coping strategies and refers to professionals when necessary.</FeatureCard>
        <FeatureCard icon={<Emoji label="calendar">📅</Emoji>} title="Confidential Booking System">Secure appointment booking with campus counsellors and emergency helplines.</FeatureCard>
        <FeatureCard icon={<Emoji label="book">📚</Emoji>} title="Resource Hub">Multilingual videos, audios, and guides accessible offline for regional students.</FeatureCard>
        <FeatureCard icon={<Emoji label="people">👥</Emoji>} title="Peer Support Forum">Moderated, anonymous peer-to-peer support with trained volunteers.</FeatureCard>
        <FeatureCard icon={<Emoji label="chart">📈</Emoji>} title="Admin Dashboard">Anonymous analytics to help institutions plan targeted interventions.</FeatureCard>
      </div>
    </section>
  );
}

function Emoji({children,label}){return <span role="img" aria-label={label} style={{fontSize:20}}>{children}</span>}
...
function Resources(){
  ...
  const fetchResults = async () => {setLoading(true);const res = await api.getResources(q);setItems(res);setLoading(false)};
  return (
    <section className="container resources" id="resources">
      <h3>Resource Hub</h3>
      <div className="search-row">
        <input placeholder="Search resources (e.g., sleep, anxiety)" value={q} onChange={(e)=>setQ(e.target.value)} />
        ...
        <button className="cta" onClick={fetchResults}>Search</button>
      </div>
        ...
        <div className="resource-grid">
          {items.map(r=> (
            <div key={r.id} className="resource-card">
              ...
              <div style={{color:'var(--muted)',fontSize:13,marginTop:6}}>{r.type} • {r.lang}</div>
            </div>
          ))}
        </div>
      )}
...
function PeerForumPreview(){
  ...
  return (
    <section className="container forum" id="peer">
      ...
      <p style={{color:'var(--muted)'}}>A safe space for moderated, anonymous conversations.</p>
...
function AdminPreview(){
  ...
  return (
    <section className="container admin" id="admin">
      ...
      <p style={{color:'var(--muted)'}}>Anonymous institutional analytics to identify trends and plan interventions.</p>
      <div className="chart-row" style={{marginTop:12}}>
        <div className="chart">
          <strong>Weekly screenings</strong>
          <div style={{height:90,display:'flex',alignItems:'end',gap:8,marginTop:12}}>
            {data.map((v,i)=> (
              <div key={i} style={{flex:1}}>
                <div style={{height:Math.max(6,v*2),background:'linear-gradient(180deg,var(--accent),#0bbf86)',borderRadius:6}} />
      ...
      <div className="container">
        <div className="footer-grid">
          <div>
            ...
            <div style={{color:'var(--muted)',marginTop:8}}>Open-source mental health support platform for colleges.</div>
          </div>
          <div>
            <strong>Quick Links</strong>
            <div style={{color:'var(--muted)',marginTop:8}}>Features<br/>Resources<br/>Peer Support</div>
          </div>
          <div>
            <strong>Contact</strong>
            <div style={{color:'var(--muted)',marginTop:8}}>studentwelfare@college.edu<br/>+91 98765 43210</div>
          </div>
          <div>
            <strong>Helplines</strong>
            <div style={{color:'var(--muted)',marginTop:8}}>24/7 Helpline: 0800-XXX-XXXX</div>
          </div>
        </div>
      </div>
    </footer>
...
export default function App(){
  ...
  const handleBook = async () => {
    const res = await api.bookAppointment({student:'anonymous',when:'ASAP'});
    ...
    <div>
      ...
      <main>
        ...
        <div className="container" style={{marginTop:18}}>
          <button className="cta" onClick={handleBook}>Book Confidential Counselling</button>
          {showBookingMsg && <div style={{marginTop:8,color:'var(--muted)'}}>{showBookingMsg}</div>}
        </div>
        <AdminPreview />
      </main>
      <Footer />
